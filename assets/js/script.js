...
function currentWeather(userInput) {
    ...
    $.ajax({
        ...
        var UVindexURL = "https://api.openweathermap.org/data/2.5/uvi?" + "lat=" + lat + "&" + "lon=" + lon + "&APPID=fb3dd2a5acdd03a900a040c7940d4846";
        ...
        cityResultText.text(cityInfo + ", " + country + ".   " + (moment(today).format("MMM Do YYYY")));
...
function forecast (userInput) {
    ...
    $.ajax({
        ...
        for (var i = 0; i < response.list.length; i += 8){
            ...
            var newH5 = $("<h5>").attr("class", "card-title").text(moment(forecastDate[i]).format("MM/DD/YY"));
